...
<body onload="initClock()" style="background-color: #ffb242;">
    ...
    if (isset($_POST['submit1'])) {
        ...
        $select_seat = "select * from seat where Seat_cat_id='$_SESSION[clickes]' and Train_id='$_SESSION[clicket]'";
        if ($select_q = mysqli_query($con, $select_seat)) {
            echo $count_select = mysqli_num_rows($select_q);
        } else {
            echo mysqli_error($con);
        }
        $arr_select = mysqli_fetch_assoc($select_q);
        while ($arr_select) {

            $arr_select = mysqli_fetch_assoc($select_q);
        }

        // $arr_select['set_seat'] =$arr_select['Seat_no'];

        // $arr_select['set_seat'] = $arr_select['set_seat'] + 1;
